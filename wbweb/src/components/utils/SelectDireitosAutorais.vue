...
export default {
    ...
    computed : {
        internalLabel (){
            ...
            return this.label != '' ? this.label : 'Direitos autorais'
        ...
        carregarDados(){
            ...
            .then((response) => {
                ...
                for(let item of response.data.data){
                    that.items.push({label:item.tipo_autoral,value:item.id})
                    that.filterItems.push({label:item.tipo_autoral,value:item.id})
                }
            })
        },
